...
var AudioDefaultOptions = /*#__PURE__*/function () {
  ...
  _createClass(AudioDefaultOptions, null, [{
     ...
     */
    function get() {
      return _objectSpread2({}, instanceOptions$1);
    }
    /**
     * @getter src
     * @returns {Object}
...
if (s$3.getItem('amst__timeWidth')) {
  ...
  var measureTime = d$1.createElement('div'),
...
var style = d$1.createElement("style"); // WebKit hack
...
var AudioUI = /*#__PURE__*/function () {
  ...
  function AudioUI(player) {
    ...
    var additionalControls = $(d$1.createElement('div'));
...
var AmstramgramAudioPlayer = /*#__PURE__*/function (_EventEmitter) {
  ...
  _createClass(AmstramgramAudioPlayer, [{
    ...
    set: function set(src) {
      ...
      */
      ...
      var rightSrc = params.src.find(function (el) {
        return el.quality && el.quality == s$2.getItem("amst__".concat(type, "DefaultQuality"));
      }) || params.src.find(function (el) {
        return el.default == true;
      }) || params.src[0]; //Updating the defaultQuality stored in sessionStorage

      if (rightSrc.quality && rightSrc.quality != s$2.getItem("amst__".concat(type, "DefaultQuality"))) {
        s$2.setItem("amst__".concat(type, "DefaultQuality"), rightSrc.quality);
    ...
    */
    ...
    ,
    set: function set(opt) {
      new Array(_classStaticPrivateFieldSpecGet(this, AmstramgramAudioPlayer, _instanceOptions$1), _classStaticPrivateFieldSpecGet(this, AmstramgramAudioPlayer, _srcOptions$1)).forEach(function (obj) {
        mergeDeep(obj, opt);
      });
    }
  }, {
...
var instanceOptions = _objectSpread2(_objectSpread2({}, AudioDefaultOptions.instance), {}, {
  appLabel: 'Video player',
  touchSeekingInfoText: 'Tap & go !'
}),
    srcOptions = _objectSpread2(_objectSpread2({}, AudioDefaultOptions.src), {}, {
  format: 16 / 9,
  fullscreen: {
    label: {
      on: 'Exit fullscreen',
...
var VideoDefaultOptions = /*#__PURE__*/function () {
  function VideoDefaultOptions() {
    _classCallCheck(this, VideoDefaultOptions);
  }

  _createClass(VideoDefaultOptions, null, [{
    key: "instance",
    get:
    /**
     * @getter instance
...
var PipButton = /*#__PURE__*/function (_ToggleButton) {
  _inherits(PipButton, _ToggleButton);

  var _super = _createSuper(PipButton);
   ...
   * @param {HTMLElement} ui - ui container to which the button will be added
   */
  function PipButton(player, ui) {
    var _this;

    _classCallCheck(this, PipButton);

    //Call the toggleButton constructor
    _this = _super.call(this, 'pip', player, ui);
    var media = player.params.media; //The button should be enabled only after metadata are loaded

    media.on('loadedmetadata', function (_) {
      if (!_this.params.disabled) _this.button.removeAttribute('data-disabled');
    }).on('enterpictureinpicture', function (_) {
      return _this.state = true;
    }).on('leavepictureinpicture', function (_) {
      return _this.state = false;
    });
    player.on('amst__src-change', function (_) {
      if (_this.state) exitPIP();
      setTimeout(function (_) {
        return _this.button.setAttribute('data-disabled', true);
      }, 1);
    }).on('amst__pip-click', function (b) {
      if (b.state) {
        //Exit PIP
        exitPIP();
      } else {
        //Enter PIP
        media.requestPictureInPicture().catch(function (_) {});
      }
    });

    function exitPIP() {
      document.exitPictureInPicture().catch(function (_) {});
    }
  ...
  _createClass(PipButton, [{
    key: "update",
    value: function update() {
      _get(_getPrototypeOf(PipButton.prototype), "update", this).call(this); //The button is disabled if metadata have not yet been loaded
...
var FullscreenButton = /*#__PURE__*/function (_ToggleButton) {
  _inherits(FullscreenButton, _ToggleButton);
   ...
   * @param {HTMLElement} ui - ui container to which the button will be added
   */
  function FullscreenButton(player, ui) {
    var _this;

    _classCallCheck(this, FullscreenButton);

    //Call the toggleButton constructor
    _this = _super.call(this, 'fullscreen', player, ui);

    _isFullscreen.set(_assertThisInitialized(_this), {
      writable: true,
      value: false
    });
    ...
    */
    ...
    player.on('amst__fullscreen-click', function (_) {
      if (_this.state) {
        //Exit 
    ...
    function leaveFullscreen() {
      ...
      container.setAttribute('style', "padding-bottom:".concat(1 / playerParams.format * 100, "%"));
    ...
    d.addEventListener(fullscreenAPI.fullscreenchange, function (_) {
      if (_classPrivateFieldGet(_assertThisInitialized(_this), _isFullscreen)) {
        //Exit
        //Reset the isFullscreen private property
        leaveFullscreen();

        _classPrivateFieldSet(_assertThisInitialized(_this), _isFullscreen, false);
      } else if (d[fullscreenAPI.fullscreenElement] == $()) {
        //The instance is in fullscreen state
        //Set the isFullscreen private property
        _classPrivateFieldSet(_assertThisInitialized(_this), _isFullscreen, true);
      }
...
var VideoUI = /*#__PURE__*/function (_AudioUI) {
  _inherits(VideoUI, _AudioUI);
  ...
  function VideoUI(player) {
    ...
    var d = document,
        ...
        timeSlider = _this.timeSlider,
        seekings = $$('.amst__seeking'),
        self = _assertThisInitialized(_this); //PIP button if PIP is supported
    ...
    }).on('playing volumechange', function () {
      if (params.currentPointerType != 'mouse') hideControls(true);
    }); //Player events

    player.on('amst__play amst__should-play', function (_) {
    ...
    *****************************************/
    ...
    var playerWidth; //Needed for the thumbnail position on touch device
    ...
    function resize() {
      ...
      var scale = Math.min(playerWidth, self.boundingRect.height) * 0.3 / 90; //scale must finally be constrained between 0.5 and 1

      scale = Math.max(0.4, Math.min(1, scale));

      if (scale == 1) {
        [].concat(_toConsumableArray($$('.amst__layer-scale')), _toConsumableArray($$('.amst__layer-scale > div'))).forEach(function (el) {
          return el.removeAttribute('style');
        });
      } else {
        $$('.amst__layer-scale').css({
          height: "calc(100% - ".concat(controls.css('height'), "px")
        });
      ...
      var subFontSize = Math.round(24 * playerWidth / 1000);
    ...
    *********************************************/
    ...
    player.on('amst__src-change', function (_) {
      ...
      if (params.poster) {
        layerPoster.style.backgroundImage = "url(\"".concat(params.poster, "\")");
        layerPoster.classList.remove('amst__hidden');
      } else {
      ...
      var paramsThumbnails = params.thumbnails; //Updating the thumbnails

      if (paramsThumbnails.src && paramsThumbnails.width > 0 && paramsThumbnails.int > 0) {
        var thumbnailsImage = new Image();
        ...
        function onThumbLoad(e) {
          ...
          if (e.type == 'load' && e.target.getAttribute('src') == paramsThumbnails.src) {
            ...
            var css = {
              width: "".concat(paramsThumbnails.width, "px"),
              height: "".concat(thumbnailsImage.naturalHeight, "px"),
              'background-image': "url(\"".concat(paramsThumbnails.src, "\")")
            };
    ...
    function updateThumbnails(ratio) {
      var offset = params.thumbnails.width * Math.floor(ratio * media.duration / params.thumbnails.int);
      seekings.css({
    ...
    *************************************************/
    ...
    if (params.pointerEventsInterface != 'touch') {
      controls.on(params.pointerenter, function (_) {
        if (params.currentPointerType == 'mouse') pointerOverControls = true;
      }).on(params.pointerleave, function (_) {
        if (params.currentPointerType == 'mouse') {
          hideControls(true);
          pointerOverControls = false;
        }
      });
      layerOverlay.on(params.pointermove, function (_) {
    ...
    ***********************************************************************************************/
    ...
    if (params.pointerEventsInterface != 'mouse') {
      var threshold = 20; //Minimum distance for a swipe detection
      ...
      layerOverlay.on('touchstart', function (e) {
      ...
      function onTouchMove(e) {
        ...
        ratio != undefined) {
          ...
          seekingTouchCover.style.transform = "scaleX(".concat(ratio, ")"); //Update time indicator
          ...
          var translate = ratio * playerWidth - 0.5 * seekingWidth;
          translate = Math.max(0, Math.min(playerWidth - seekingWidth, translate));
          seekingTouchWrapper.style.transform = "translateX(".concat(translate, "px)"); //Update the thumbnail
    ...
    _this.timeSlider.on('amst__time-slider-update', function (ratio) {
      $('.amst__seeking-time-display').style.transform = "scaleX(".concat(ratio, ")");
    });
    /*************************************************
    * 
    ...
    **************************************************/
    ...
    _classStaticPrivateFieldSpecGet(VideoUI, VideoUI, _videoUIs).push(_assertThisInitialized(_this));
  ...
  */
  ...
  _createClass(VideoUI, [{
    key: "reset",
    value: function reset() {
      var media = this.player.params.media,
      ...
      _get(_getPrototypeOf(VideoUI.prototype), "reset", this).call(this);
    }
  }], [{
    key: "showTouchSeekingInfo",
    get:
    /**
     * INHERITED PROPERTIES
     * @property player - AmstramgramAudioPlayer or AmstramgramVideoPlayer instance passed as parameter
    ...
    */
    ...
    key: "hideTouchSeekingInfo",
    value: function hideTouchSeekingInfo() {
      ...
      this.videoUIs.forEach(function (ui) {
        var $ = ui.player.params.$;
    ...
    */
    function get() {
      return _toConsumableArray(_classStaticPrivateFieldSpecGet(this, VideoUI, _videoUIs));
    }
  }]);
...
var AmstramgramVideoPlayer = /*#__PURE__*/function (_AmstramgramAudioPlay) {
  _inherits(AmstramgramVideoPlayer, _AmstramgramAudioPlay);

  var _super = _createSuper(AmstramgramVideoPlayer);
   ...
   * @param {Object} params - player parameters
   */
  function AmstramgramVideoPlayer(media) {
    var _this;

    var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    var callback = arguments.length > 2 ? arguments[2] : undefined;
    var WrappingClass = arguments.length > 3 ? arguments[3] : undefined;

    _classCallCheck(this, AmstramgramVideoPlayer);

    _this = _super.call(this, media, params, callback, WrappingClass);
    if (media.tagName != 'VIDEO' || !callback || typeof callback != 'function' || !WrappingClass || WrappingClass.name !== 'AmstramgramMediaPlayer') return _possibleConstructorReturn(_this);
    return _this;
  }
  ...
  _createClass(AmstramgramVideoPlayer, [{
     ...
     */
    function get() {
      return [].concat(_toConsumableArray(AmstramgramAudioPlayer.booleanAttributes), ['playsInline']);
    }
    /**
     * @getter stringAttributes
     * @returns {Array}
    ...
    key: "stringAttributes",
    get: function get() {
      return [].concat(_toConsumableArray(AmstramgramAudioPlayer.stringAttributes), ['poster']);
    }
    /**
     * @getter attributesToUpdate
     * @returns {Array}
    ...
    key: "attributesToUpdate",
    get: function get() {
      return [].concat(_toConsumableArray(AmstramgramAudioPlayer.attributesToUpdate), ['playsInline']);
    }
    /* -------------------------------------------------------------------------- */
    ...
    */
    ...
    set: function set(opt) {
      new Array(_classStaticPrivateFieldSpecGet(this, AmstramgramVideoPlayer, _instanceOptions), _classStaticPrivateFieldSpecGet(this, AmstramgramVideoPlayer, _srcOptions)).forEach(function (obj) {
        mergeDeep(obj, opt);
      });
    }
  }]);
...
var AmstramgramMediaPlayer = /*#__PURE__*/function (_EventEmitter) {
  _inherits(AmstramgramMediaPlayer, _EventEmitter);

  var _super = _createSuper(AmstramgramMediaPlayer);
   ...
   */
  function AmstramgramMediaPlayer(media) {
    var _this;

    var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    var callback = arguments.length > 2 ? arguments[2] : undefined;

    _classCallCheck(this, AmstramgramMediaPlayer);

    _this = _super.call(this);
    if (!media || media.tagName != 'AUDIO' && media.tagName != 'VIDEO') return _possibleConstructorReturn(_this);

    var self = _assertThisInitialized(_this); //For IE11
    ...
    function onPlayerInit() {
      ...
      */
      getInstanceMethodNames(this, Object.getPrototypeOf(AmstramgramAudioPlayer.prototype)).forEach(function (m) {
        var desc = {};

        if (getDescriptor(m, _this2).set) {
        ...
        if (getDescriptor(m, _this2).value) {
          //pause - play -toggle
          desc.value = function (_) {
            return _this2[m]();
          };
        }
      ...
      Object.getOwnPropertyNames(this).forEach(function (p) {
        ...
        if (Object.getOwnPropertyDescriptor(_this2, p).set) {
          //download - next - more - mute - playPause - previous - settings - subtitles - fullscreen - pip
          desc.set = function (val) {
            return _this2[p] = val;
          };
        ...
        if (Object.getOwnPropertyDescriptor(_this2, p).value) {
          //events - params
          desc.value = _this2[p];
        }
  ...
  _createClass(AmstramgramMediaPlayer, null, [{
    key: "options",
    get:
    /**
     * @get options
  ...
  return AmstramgramMediaPlayer;
}(EventEmitter);

module.exports = AmstramgramMediaPlayer;
